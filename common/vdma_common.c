...
#define VDMA_CHANNEL_NUM_PROCESSED_MASK                                        \
  ((1 << VDMA_CHANNEL_NUM_PROCESSED_WIDTH) - 1)
#define VDMA_CHANNEL_NUM_ONGOING_MASK VDMA_CHANNEL_NUM_PROCESSED_MASK

#define TIMESTAMPS_CIRC_SPACE(timestamp_list)                                  \
  CIRC_SPACE((timestamp_list).head, (timestamp_list).tail,                     \
             CHANNEL_IRQ_TIMESTAMPS_SIZE)
#define TIMESTAMPS_CIRC_CNT(timestamp_list)                                    \
  CIRC_CNT((timestamp_list).head, (timestamp_list).tail,                       \
           CHANNEL_IRQ_TIMESTAMPS_SIZE)

#define ONGOING_TRANSFERS_CIRC_SPACE(transfers_list)                           \
  CIRC_SPACE((transfers_list).head, (transfers_list).tail,                     \
             HAILO_VDMA_MAX_ONGOING_TRANSFERS)
#define ONGOING_TRANSFERS_CIRC_CNT(transfers_list)                             \
  CIRC_CNT((transfers_list).head, (transfers_list).tail,                       \
           HAILO_VDMA_MAX_ONGOING_TRANSFERS)
...
static int
ongoing_transfer_push(struct hailo_vdma_channel *channel,
  ...
  if (ongoing_transfer->dirty_descs_count >
      ARRAY_SIZE(ongoing_transfer->dirty_descs)) {
    return -EFAULT;
  }

  transfers->transfers[transfers->head] = *ongoing_transfer;
  transfers->head =
      (transfers->head + 1) & HAILO_VDMA_MAX_ONGOING_TRANSFERS_MASK;
  return 0;
}

static int
ongoing_transfer_pop(struct hailo_vdma_channel *channel,
                     struct hailo_ongoing_transfer *ongoing_transfer) {
  struct hailo_ongoing_transfers_list *transfers = &channel->ongoing_transfers;
  ...
  transfers->tail =
      (transfers->tail + 1) & HAILO_VDMA_MAX_ONGOING_TRANSFERS_MASK;
  return 0;
}

static void clear_dirty_desc(struct hailo_vdma_descriptors_list *desc_list,
                             u16 desc) {
  desc_list->desc_list[desc].PageSize_DescControl =
      (u32)((desc_list->desc_page_size << DESCRIPTOR_PAGE_SIZE_SHIFT) +
...
static void clear_dirty_descs(struct hailo_vdma_channel *channel,
  ...
  BUG_ON(ongoing_transfer->dirty_descs_count >
         ARRAY_SIZE(ongoing_transfer->dirty_descs));
...
static bool
validate_last_desc_status(struct hailo_vdma_channel *channel,
                          struct hailo_ongoing_transfer *ongoing_transfer) {
  u16 last_desc = ongoing_transfer->last_desc;
  u32 last_desc_control =
      channel->last_desc_list->desc_list[last_desc].RemainingPageSize_Status &
...
static void
hailo_vdma_program_descriptor(struct hailo_vdma_descriptor *descriptor,
                              u64 dma_address, size_t page_size, u8 data_id) {
  descriptor->PageSize_DescControl =
      (u32)((page_size << DESCRIPTOR_PAGE_SIZE_SHIFT) +
            DESCRIPTOR_DESC_CONTROL);
  descriptor->AddrL_rsvd_DataID =
      (u32)(((dma_address & DESCRIPTOR_ADDR_L_MASK)) | data_id);
  descriptor->AddrH = (u32)(dma_address >> 32);
  descriptor->RemainingPageSize_Status = 0;
}

static u8 get_channel_id(u8 channel_index) {
  return (channel_index < MAX_VDMA_CHANNELS_PER_ENGINE) ? (channel_index & 0xF)
                                                        : INVALID_VDMA_CHANNEL;
}

int hailo_vdma_program_descriptors_in_chunk(
    struct hailo_vdma_hw *vdma_hw, dma_addr_t chunk_addr,
    unsigned int chunk_size, struct hailo_vdma_descriptors_list *desc_list,
    u32 desc_index, u32 max_desc_index, u8 channel_index, u8 data_id) {
  ...
  if (INVALID_VDMA_ADDRESS == encoded_addr) {
    pr_err("INVALID_VDMA_ADDRESS == encoded_addr: %llx", chunk_addr);
    return -EFAULT;
  }

  // Program all descriptors except the last one
  for (desc_index = starting_desc_index;
       desc_index < starting_desc_index + descs_to_program - 1; desc_index++) {
    // 'desc_index & desc_list_len_mask' is used instead of modulo; see
    // hailo_vdma_descriptors_list documentation.
...
get_interrupts_bitmask(struct hailo_vdma_hw *vdma_hw,
...
static int bind_and_program_descriptors_list(
    ...
    struct hailo_vdma_descriptors_list *desc_list, u32 starting_desc,
    struct hailo_vdma_mapped_transfer_buffer *buffer, u8 channel_index,
    enum hailo_vdma_interrupts_domain last_desc_interrupts, bool is_debug) {
  ...
  if (starting_desc >= desc_list->desc_count) {
    pr_err("starting_desc >= desc_list->desc_count");
  ...
  pr_err("Bind descriptors prog-size %lld max_desc_index %lld",
         (long long)program_size, (long long)max_desc_index);
  for_each_sgtable_dma_sg(buffer->sg_table, sg_entry, i) {
    ...
    pr_err("chunk_start_addr %lld chunk_size %lld", (long long)chunk_start_addr,
  ...
  if (program_size != 0) {
    pr_err("program_size != 0 %lld", (long long)program_size);
...
static int program_last_desc(
    ...
    enum hailo_vdma_interrupts_domain last_desc_interrupts, bool is_debug) {
  u8 control =
      (u8)(DESCRIPTOR_DESC_CONTROL |
           get_interrupts_bitmask(vdma_hw, last_desc_interrupts, is_debug));
  u32 total_descs =
      DIV_ROUND_UP(transfer_buffer->size, desc_list->desc_page_size);
  u32 last_desc = (starting_desc + total_descs - 1) % desc_list->desc_count;
  u32 last_desc_size =
      transfer_buffer->size - (total_descs - 1) * desc_list->desc_page_size;

  // Configure only last descriptor with residue size
  desc_list->desc_list[last_desc].PageSize_DescControl =
      (u32)((last_desc_size << DESCRIPTOR_PAGE_SIZE_SHIFT) + control);
...
int hailo_vdma_program_descriptors_list(
    struct hailo_vdma_hw *vdma_hw,
    struct hailo_vdma_descriptors_list *desc_list, u32 starting_desc,
    struct hailo_vdma_mapped_transfer_buffer *buffer, bool should_bind,
    u8 channel_index, enum hailo_vdma_interrupts_domain last_desc_interrupts,
    bool is_debug) {
  return should_bind
             ? bind_and_program_descriptors_list(
                   vdma_hw, desc_list, starting_desc, buffer, channel_index,
...
static bool channel_control_reg_is_active(u8 control) {
  return (control & VDMA_CHANNEL_CONTROL_START_ABORT_BITMASK) ==
         VDMA_CHANNEL_CONTROL_START;
}

static int validate_channel_state(struct hailo_vdma_channel *channel) {
  u32 host_regs_value = ioread32(channel->host_regs);
  const u8 control = READ_BITS_AT_OFFSET(BYTE_SIZE * BITS_IN_BYTE,
                                         CHANNEL_CONTROL_OFFSET * BITS_IN_BYTE,
                                         host_regs_value);
  const u16 hw_num_avail = READ_BITS_AT_OFFSET(
      WORD_SIZE * BITS_IN_BYTE, CHANNEL_NUM_AVAIL_OFFSET * BITS_IN_BYTE,
      host_regs_value);
  ...
  if (hw_num_avail != channel->state.num_avail) {
    pr_err(
        "Channel %d hw state out of sync. num available is %d, expected %d\n",
...
void hailo_vdma_set_num_avail(u8 __iomem *regs, u16 num_avail) {
  ...
  iowrite32(WRITE_BITS_AT_OFFSET(WORD_SIZE * BITS_IN_BYTE,
                                 CHANNEL_NUM_AVAIL_OFFSET * BITS_IN_BYTE,
                                 regs_val, num_avail),
            regs);
}

u16 hailo_vdma_get_num_proc(u8 __iomem *regs) {
  return READ_BITS_AT_OFFSET(WORD_SIZE * BITS_IN_BYTE, 0,
                             ioread32(regs + CHANNEL_NUM_PROC_OFFSET));
}

int hailo_vdma_launch_transfer(
    struct hailo_vdma_hw *vdma_hw, struct hailo_vdma_channel *channel,
    struct hailo_vdma_descriptors_list *desc_list, u32 starting_desc,
    u8 buffers_count, struct hailo_vdma_mapped_transfer_buffer *buffers,
    bool should_bind, enum hailo_vdma_interrupts_domain first_interrupts_domain,
    enum hailo_vdma_interrupts_domain last_desc_interrupts, bool is_debug) {
  int ret = -EFAULT;
  u32 total_descs = 0;
  ...
  if (ret < 0) {
    ...
    pr_err("Validate channel failed %d\n", channel->index);
    return ret;
  }
  ...
  BUILD_BUG_ON_MSG((HAILO_MAX_BUFFERS_PER_SINGLE_TRANSFER + 1) !=
                       ARRAY_SIZE(ongoing_transfer.dirty_descs),
  ...
  for (i = 0; i < buffers_count; i++) {
    pr_err("buffer %d\n", i);
    ret = hailo_vdma_program_descriptors_list(
        vdma_hw, desc_list, starting_desc, &buffers[i], should_bind,
        channel->index,
        (i == (buffers_count - 1) ? last_desc_interrupts
...
static bool hailo_vdma_pop_timestamp(
  ...
  *out_timestamp = timestamp_list->timestamps[timestamp_list->tail];
  timestamp_list->tail =
      (timestamp_list->tail + 1) & CHANNEL_IRQ_TIMESTAMPS_SIZE_MASK;
  return true;
}

static void hailo_vdma_pop_timestamps_to_response(
  ...
  while (hailo_vdma_pop_timestamp(&channel->timestamp_list,
                                  &result->timestamps[i]) &&
         (i < max_timestamps)) {
    ...
    // zero.
    result->timestamps[i].desc_num_processed =
        result->timestamps[i].desc_num_processed &
...
static u8 __iomem *get_channel_regs(u8 __iomem *regs_base, u8 channel_index,
                                    bool is_host_side,
                                    u32 src_channels_bitmask) {
  // Check if getting host side regs or device side
  u8 __iomem *channel_regs_base =
      regs_base + CHANNEL_BASE_OFFSET(channel_index);
  if (is_host_side) {
    return hailo_test_bit(channel_index, &src_channels_bitmask)
               ? channel_regs_base
...
void hailo_vdma_engine_init(struct hailo_vdma_engine *engine, u8 engine_index,
  ...
  for_each_vdma_channel(engine, channel, channel_index) {
    ...
    channel->host_regs =
        get_channel_regs(regs_base, channel_index, true, src_channels_bitmask);
    channel->device_regs =
        get_channel_regs(regs_base, channel_index, false, src_channels_bitmask);
...
/**
 * Enables the given channels bitmap in the given engine. Allows launching
 * transfer and reading interrupts from the channels.
 *
 ...
 */
void hailo_vdma_engine_enable_channels(struct hailo_vdma_engine *engine,
                                       u32 bitmap, bool measure_timestamp) {
 ...
 * @param measure_timestamp - if set, allow interrupts timestamp measure.
 */
void hailo_vdma_engine_disable_channels(struct hailo_vdma_engine *engine,
                                        u32 bitmap) {
  struct hailo_vdma_channel *channel = NULL;
  u8 channel_index = 0;
...
void hailo_vdma_engine_push_timestamps(struct hailo_vdma_engine *engine,
                                       u32 bitmap) {
  struct hailo_vdma_channel *channel = NULL;
  u8 channel_index = 0;
...
int hailo_vdma_engine_read_timestamps(
...
void hailo_vdma_engine_clear_channel_interrupts(
    struct hailo_vdma_engine *engine, u32 bitmap) {
  engine->interrupted_channels &= ~bitmap;
}

void hailo_vdma_engine_set_channel_interrupts(struct hailo_vdma_engine *engine,
                                              u32 bitmap) {
  engine->interrupted_channels |= bitmap;
}

static void
fill_channel_irq_data(struct hailo_vdma_interrupts_channel_data *irq_data,
                      struct hailo_vdma_engine *engine,
                      struct hailo_vdma_channel *channel,
                      u8 transfers_completed, bool validation_success) {
  u8 host_control = READ_BITS_AT_OFFSET(BYTE_SIZE * BITS_IN_BYTE,
                                        CHANNEL_CONTROL_OFFSET * BITS_IN_BYTE,
                                        ioread32(channel->host_regs));
  u8 device_control = READ_BITS_AT_OFFSET(BYTE_SIZE * BITS_IN_BYTE,
                                          CHANNEL_CONTROL_OFFSET * BITS_IN_BYTE,
                                          ioread32(channel->device_regs));
  ...
  irq_data->host_error =
      READ_BITS_AT_OFFSET(BYTE_SIZE * BITS_IN_BYTE, 0,
                          ioread32(channel->host_regs + CHANNEL_ERROR_OFFSET));
  irq_data->device_error = READ_BITS_AT_OFFSET(
      BYTE_SIZE * BITS_IN_BYTE, 0,
      ioread32(channel->device_regs + CHANNEL_ERROR_OFFSET));
...
static bool is_transfer_complete(struct hailo_vdma_channel *channel,
  ...
  return is_desc_between(channel->state.num_proc, hw_num_proc,
                         transfer->last_desc);
}
...
int hailo_vdma_engine_fill_irq_data(
  ...
  for_each_vdma_channel(engine, channel, channel_index) {
    ...
    BUILD_BUG_ON_MSG(HAILO_VDMA_MAX_ONGOING_TRANSFERS >= U8_MAX,
                     "HAILO_VDMA_MAX_ONGOING_TRANSFERS must be less than "
                     "U8_MAX to use transfers_completed as u8");
    ...
    while (ONGOING_TRANSFERS_CIRC_CNT(channel->ongoing_transfers) > 0) {
      struct hailo_ongoing_transfer *cur_transfer =
          &channel->ongoing_transfers
               .transfers[channel->ongoing_transfers.tail];
      ...
      channel->state.num_proc =
          (u16)((cur_transfer->last_desc + 1) & channel->state.desc_count_mask);

      ongoing_transfer_pop(channel, NULL);
      transfers_completed++;
    }

    fill_channel_irq_data(&irq_data->irq_data[irq_data->channels_count], engine,
                          channel, transfers_completed, validation_success);
...
// For all these functions - best way to optimize might be to not call the
// function when need to pause and then abort, Rather read value once and maybe
// save This function reads and writes the register - should try to make more
// optimized in future
static void start_vdma_control_register(u8 __iomem *host_regs) {
  ...
  iowrite32(WRITE_BITS_AT_OFFSET(
                BYTE_SIZE * BITS_IN_BYTE, CHANNEL_CONTROL_OFFSET * BITS_IN_BYTE,
                host_regs_value, VDMA_CHANNEL_CONTROL_START_RESUME),
            host_regs);
}
...
static void hailo_vdma_channel_pause(u8 __iomem *host_regs) {
  ...
  iowrite32(WRITE_BITS_AT_OFFSET(
                BYTE_SIZE * BITS_IN_BYTE, CHANNEL_CONTROL_OFFSET * BITS_IN_BYTE,
                host_regs_value, VDMA_CHANNEL_CONTROL_START_PAUSE),
            host_regs);
}

// This function reads and writes the register - should try to make more
// optimized in future
static void hailo_vdma_channel_abort(u8 __iomem *host_regs) {
  u32 host_regs_value = ioread32(host_regs);
  iowrite32(WRITE_BITS_AT_OFFSET(BYTE_SIZE * BITS_IN_BYTE,
                                 CHANNEL_CONTROL_OFFSET * BITS_IN_BYTE,
                                 host_regs_value, VDMA_CHANNEL_CONTROL_ABORT),
            host_regs);
}

int hailo_vdma_start_channel(u8 __iomem *regs, uint64_t desc_dma_address,
                             uint32_t desc_count, uint8_t data_id) {
  u16 dma_address_l = 0;
  u32 dma_address_h = 0;
  ...
  iowrite32(WRITE_BITS_AT_OFFSET(
                WORD_SIZE * BITS_IN_BYTE,
                (VDMA_CHANNEL__ADDRESS_L_OFFSET -
                 VDMA_CHANNEL__ALIGNED_ADDRESS_L_OFFSET) *
                    BITS_IN_BYTE,
                ioread32(regs + VDMA_CHANNEL__ALIGNED_ADDRESS_L_OFFSET),
                dma_address_l),
            regs + VDMA_CHANNEL__ALIGNED_ADDRESS_L_OFFSET);
...
static bool hailo_vdma_channel_is_idle(u8 __iomem *host_regs,
                                       size_t host_side_max_desc_count) {
  // Num processed and ongoing are next to each other in the memory.
  // Reading them both in order to save BAR reads.
  u32 host_side_num_processed_ongoing =
      ioread32(host_regs + CHANNEL_NUM_PROC_OFFSET);
  u16 host_side_num_processed =
      (host_side_num_processed_ongoing & VDMA_CHANNEL_NUM_PROCESSED_MASK);
  u16 host_side_num_ongoing =
      (host_side_num_processed_ongoing >> VDMA_CHANNEL_NUM_PROCESSED_WIDTH) &
      VDMA_CHANNEL_NUM_ONGOING_MASK;

  if ((host_side_num_processed % host_side_max_desc_count) ==
      (host_side_num_ongoing % host_side_max_desc_count)) {
    return true;
  }
...
static int hailo_vdma_wait_until_channel_idle(u8 __iomem *host_regs) {
  ...
  u8 depth = (uint8_t)(READ_BITS_AT_OFFSET(VDMA_CHANNEL_DESC_DEPTH_WIDTH,
                                           VDMA_CHANNEL_DESC_DEPTH_SHIFT,
                                           ioread32(host_regs)));
  size_t host_side_max_desc_count = (size_t)(1 << depth);

  for (check_counter = 0;
       check_counter < VDMA_CHANNEL__MAX_CHECKS_CHANNEL_IS_IDLE;
       check_counter++) {
...
void hailo_vdma_stop_channel(u8 __iomem *regs) {
  int err = 0;
  u8 host_side_channel_regs = READ_BITS_AT_OFFSET(
      BYTE_SIZE * BITS_IN_BYTE, CHANNEL_CONTROL_OFFSET * BITS_IN_BYTE,
      ioread32(regs));

  if ((host_side_channel_regs &
       VDMA_CHANNEL_CONTROL_START_ABORT_PAUSE_RESUME_BITMASK) ==
      VDMA_CHANNEL_CONTROL_ABORT_PAUSE) {
    // The channel is aborted (we set the channel to
    // VDMA_CHANNEL_CONTROL_ABORT_PAUSE at the end of this function)
    return;
  }

  // Pause the channel
  // The channel is paused to allow for "all transfers from fetched
  // descriptors..." to be "...completed" (from PLDA PCIe refernce manual,
  // "9.2.5 Starting a Channel and Transferring Data")
  hailo_vdma_channel_pause(regs);

  // Even if channel is stuck and not idle, force abort and return error in the
  // end
  ...
  // Abort the channel (even of hailo_vdma_wait_until_channel_idle function
  // fails)
  hailo_vdma_channel_abort(regs);
}

bool hailo_check_channel_index(u8 channel_index, u32 src_channels_bitmask,
                               bool is_input_channel) {
  return is_input_channel
