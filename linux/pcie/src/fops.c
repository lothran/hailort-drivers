...
static struct hailo_file_context *
create_file_context(struct hailo_pcie_board *board, struct file *filp) {
...
static struct hailo_file_context *
find_file_context(struct hailo_pcie_board *board, struct file *filp) {
...
int hailo_pcie_fops_open(struct inode *inode, struct file *filp) {
  ...
  // allow multiple processes to open a device, count references in
  // hailo_pcie_get_board_index.
  if (!(pBoard = hailo_pcie_get_board_index(minor))) {
    pr_err(DRIVER_NAME
           ": fops_open: PCIe board not found for /dev/hailo%d node.\n",
           minor);
    err = -ENODEV;
    goto l_exit;
  }
  ...
  hailo_dbg(pBoard, "(%d: %d-%d): fops_open: SUCCESS on /dev/hailo%d\n",
            current->tgid, major, minor, minor);
...
int hailo_pcie_fops_release(struct inode *inode, struct file *filp) {
  struct hailo_pcie_board *board =
      (struct hailo_pcie_board *)filp->private_data;
  struct hailo_file_context *context = NULL;
  ...
  if (board) {
    ...
    if (false == context->is_valid) {
      // File context is invalid, but open. It's OK to continue finalize and
      // release it.
    ...
    hailo_vdma_file_context_finalize(&context->vdma_context, &board->vdma,
                                     filp);
    release_file_context(context);
...
static long hailo_memory_transfer_ioctl(struct hailo_pcie_board *board,
                                        unsigned long arg) {
  long err = 0;
  struct hailo_memory_transfer_params *transfer =
      &board->memory_transfer_params;
...
static void firmware_notification_irq_handler(struct hailo_pcie_board *board) {
  ...
  err = hailo_pcie_read_firmware_notification(&board->pcie_resources.fw_access,
                                              &board->nnc.notification_cache);
  spin_unlock_irqrestore(&board->nnc.notification_read_spinlock,
                         irq_saved_flags);
  ...
  if (err < 0) {
    ...
    rcu_read_lock();
    list_for_each_entry_rcu(notif_wait_cursor,
                            &board->nnc.notification_wait_list,
                            notification_wait_list) {
...
static void boot_irq_handler(struct hailo_pcie_board *board,
                             struct hailo_pcie_interrupt_source *irq_source) {
...
static void nnc_irq_handler(struct hailo_pcie_board *board,
                            struct hailo_pcie_interrupt_source *irq_source) {
...
static void soc_irq_handler(struct hailo_pcie_board *board,
                            struct hailo_pcie_interrupt_source *irq_source) {
  ...
  if (irq_source->sw_interrupts & HAILO_PCIE_SOC_CLOSE_IRQ) {
    ...
    // always use bitmap=0xFFFFFFFF - it is ok to wake all interrupts since each
    // handler will check if the stream was aborted or not.
    hailo_vdma_wakeup_interrupts(
        &board->vdma, &board->vdma.vdma_engines[DEFAULT_VDMA_ENGINE_INDEX],
        0xFFFFFFFF);
  }
}
...
#endif
{
  ...
  while (true) {
    ...
    got_interrupt =
        hailo_pcie_read_interrupt(&board->pcie_resources, &irq_source);
    if (!got_interrupt) {
      break;
    }
...
static long hailo_query_device_properties(struct hailo_pcie_board *board,
                                          unsigned long arg) {
...
static long hailo_query_driver_info(struct hailo_pcie_board *board,
                                    unsigned long arg) {
  struct hailo_driver_info info = {.major_version = HAILO_DRV_VER_MAJOR,
                                   .minor_version = HAILO_DRV_VER_MINOR,
                                   .revision_version = HAILO_DRV_VER_REVISION};

  hailo_info(board,
             "HAILO_QUERY_DRIVER_INFO: major=%u, minor=%u, revision=%u\n",
...
static long hailo_general_ioctl(struct hailo_pcie_board *board,
...
long hailo_pcie_fops_unlockedioctl(struct file *filp, unsigned int cmd,
                                   unsigned long arg) {
  long err = 0;
  struct hailo_pcie_board *board =
      (struct hailo_pcie_board *)filp->private_data;
  ...
  if (_IOC_DIR(cmd) & _IOC_READ) {
    err =
        !compatible_access_ok(VERIFY_WRITE, (void __user *)arg, _IOC_SIZE(cmd));
  } else if (_IOC_DIR(cmd) & _IOC_WRITE) {
    err =
        !compatible_access_ok(VERIFY_READ, (void __user *)arg, _IOC_SIZE(cmd));
  }
  ...
  case HAILO_VDMA_IOCTL_MAGIC:
    err = hailo_vdma_ioctl(&context->vdma_context, &board->vdma, cmd, arg, filp,
                           &board->mutex, &should_up_board_mutex);
    break;
...
int hailo_pcie_fops_mmap(struct file *filp, struct vm_area_struct *vma) {
  ...
  BUILD_BUG_ON_MSG(sizeof(vma->vm_pgoff) < sizeof(vdma_handle),
                   "If this expression fails to compile it means the target HW "
                   "is not compatible with our approach to use "
                   "the page offset paramter of 'mmap' to pass the driver the "
                   "'handle' of the desired descriptor");

  vma->vm_pgoff = 0; // vm_pgoff contains vdma_handle page offset, the actual
                     // offset from the phys addr is 0
