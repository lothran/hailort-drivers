...
// enum that represents values for the driver parameter to either force buffer
// from driver , userspace or not force and let driver decide
...
/**
 * Due to an HW bug, on system with low MaxReadReq ( < 512) we need to use
 * different descriptors size. Returns the max descriptor size or 0 on failure.
 */
static int hailo_get_desc_page_size(struct pci_dev *pdev, u32 *out_page_size) {
  u16 pcie_device_control = 0;
  int err = 0;
  // The default page size must be smaller/equal to 32K (due to PLDA registers
  // limit).
  ...
  if (force_desc_page_size != 0) {
    ...
    if (force_desc_page_size > max_page_size) {
      pci_err(pdev, "force_desc_page_size %d mustn't be larger than %u",
              force_desc_page_size, max_page_size);
      return -EINVAL;
    }

    pci_notice(pdev,
               "Probing: Force setting max_desc_page_size to %d (recommended "
               "value is %lu)\n",
  ...
  case PCI_EXP_DEVCTL_READRQ_128B:
    pci_notice(pdev,
               "Probing: Setting max_desc_page_size to 128 (recommended value "
               "is %u)\n",
               defualt_page_size);
    *out_page_size = 128;
    return 0;
  case PCI_EXP_DEVCTL_READRQ_256B:
    pci_notice(pdev,
               "Probing: Setting max_desc_page_size to 256 (recommended value "
               "is %u)\n",
               defualt_page_size);
    *out_page_size = 256;
    return 0;
  default:
    pci_notice(pdev,
               "Probing: Setting max_desc_page_size to %u, (page_size=%lu)\n",
 ...
 **/
static int hailo_pcie_disable_aspm(struct hailo_pcie_board *board, u16 state,
                                   bool locked) {
  ...
  // Disable L0s even if it is currently disabled as ASPM states can be enabled
  // by the kernel when changing power modes
#ifdef CONFIG_PCIEASPM
  if (locked) {
    // Older kernel versions (<5.2.21) don't return value for this functions, so
    // we try manual disabling anyway
...
static void hailo_pcie_insert_board(struct hailo_pcie_board *pBoard) {
  ...
  if (list_empty(&g_hailo_board_list) ||
      list_first_entry(&g_hailo_board_list, struct hailo_pcie_board, board_list)
              ->board_index > 0) {
  ...
  list_for_each_entry_safe(pCurrent, pNext, &g_hailo_board_list, board_list) {
    ...
    if (list_is_last(&pCurrent->board_list, &g_hailo_board_list) ||
        (index != pNext->board_index)) {
      break;
    }
  }
 ...
 */
static bool wait_for_firmware_completion(struct completion *completion,
                                         unsigned int msecs) {
  return (0 !=
          wait_for_completion_timeout(completion, msecs_to_jiffies(msecs)));
}
 ...
 * @param boot_dma_state - pointer to the boot dma state struct which includes
 * all of the boot resources.
 ...
 * @param raise_int_on_completion - true if this is the last descriptors chunk
 * in the specific channel in the boot flow, false otherwise. If true - will
 * enable an IRQ for the relevant channel when the transfer is finished.
 * @return the amount of descriptors programmed on success, negative error code
 * on failure.
 */
static int pcie_vdma_program_one_file_descriptors(
    struct device *dev,
    struct hailo_pcie_boot_dma_channel_state *boot_channel_state,
    u32 file_address, struct hailo_vdma_mapped_transfer_buffer transfer_buffer,
    u8 channel_index, const char *filename, bool raise_int_on_completion) {
  int device_desc = 0, host_desc = 0;
  enum hailo_vdma_interrupts_domain interrupts_domain =
      raise_int_on_completion ? HAILO_VDMA_INTERRUPTS_DOMAIN_HOST
                              : HAILO_VDMA_INTERRUPTS_DOMAIN_NONE;

  hailo_dev_dbg(
      dev,
      "channel_index = %d, file_name = %s, file_address = 0x%x, transfer_buffer.offset = 0x%x,\
        size_to_program = 0x%x, starting_desc/desc_index = 0x%x\n",
      channel_index, filename, file_address, transfer_buffer.offset,
      transfer_buffer.size, boot_channel_state->desc_program_num);

  // program descriptors
  device_desc = hailo_vdma_program_descriptors_in_chunk(
      &hailo_pcie_vdma_hw, file_address, transfer_buffer.size,
      &boot_channel_state->device_descriptors_buffer.desc_list,
      boot_channel_state->desc_program_num,
      (boot_channel_state->device_descriptors_buffer.desc_list.desc_count - 1),
      channel_index, HAILO_PCI_EP_HOST_DMA_DATA_ID);
  if (device_desc < 0) {
  ...
  host_desc = hailo_vdma_program_descriptors_list(
      &hailo_pcie_vdma_hw,
      &boot_channel_state->host_descriptors_buffer.desc_list,
      boot_channel_state->desc_program_num, &transfer_buffer, true,
      channel_index, interrupts_domain, false);
  if (host_desc < 0) {
  ...
  // checks that same amount of decsriptors were programmed on device side and
  // host side
 ...
 * @param file_address - the address of the file in the device memory.
 * @param filename - the name of the file to program.
 * @param raise_int_on_completion - true if this is the last file in the boot
 * flow, false otherwise. uses to enable an IRQ for the relevant channel when
 * the transfer is finished.
 * @return 0 on success, negative error code on failure. at the end of the
 * function the firmware is released.
 */
static int
pcie_vdma_program_one_file(struct hailo_pcie_board *board,
                           struct hailo_pcie_boot_dma_state *boot_dma_state,
                           u32 file_address, const char *filename,
                           bool raise_int_on_completion) {
  const struct firmware *firmware = NULL;
  struct hailo_vdma_mapped_transfer_buffer transfer_buffer = {0};
  int desc_programmed = 0;
  int err = 0;
  size_t bytes_copied = 0, remaining_size = 0, data_offset = 0,
         desc_num_left = 0, current_desc_to_program = 0;
  ...
  while (remaining_size > 0) {
    struct hailo_pcie_boot_dma_channel_state *channel =
        &boot_dma_state->channels[boot_dma_state->curr_channel_index];
    bool is_last_desc_chunk_of_curr_channel = false;
    bool rais_interrupt_on_last_chunk = false;

    hailo_dbg(
        board,
        "desc_program_num = 0x%x, desc_page_size = 0x%x, on channel = %d\n",
        channel->desc_program_num, HAILO_PCI_OVER_VDMA_PAGE_SIZE,
    ...
    if ((MAX_SG_DESCS_COUNT - 1) == channel->desc_program_num) {
      ...
      board->fw_boot.boot_used_channel_bitmap |=
          (1 << boot_dma_state->curr_channel_index);
    }

    // calculate the number of descriptors left to program and the number of
    ...
    transfer_buffer.size =
        min(remaining_size, (desc_num_left * HAILO_PCI_OVER_VDMA_PAGE_SIZE));
    // no need to check for overflow since the variables are constant and always
    // desc_program_num <= max u16 (65536) & the buffer max size is 256 Mb << 4G
    // (max u32)
    transfer_buffer.offset =
        (channel->desc_program_num * HAILO_PCI_OVER_VDMA_PAGE_SIZE);

    // check if this is the last descriptor chunk to program in the whole boot
    // flow
    current_desc_to_program =
        (transfer_buffer.size / HAILO_PCI_OVER_VDMA_PAGE_SIZE);
    is_last_desc_chunk_of_curr_channel =
        ((MAX_SG_DESCS_COUNT - 1) ==
         (current_desc_to_program + channel->desc_program_num));
    rais_interrupt_on_last_chunk =
        (is_last_desc_chunk_of_curr_channel ||
         (raise_int_on_completion && (remaining_size == transfer_buffer.size)));

    // try to copy the file to the buffer, if failed, release the firmware and
    // return
    bytes_copied = sg_pcopy_from_buffer(
        transfer_buffer.sg_table->sgl, transfer_buffer.sg_table->orig_nents,
    ...
    desc_programmed = pcie_vdma_program_one_file_descriptors(
        &board->pDev->dev, channel, (file_address + data_offset),
        transfer_buffer, boot_dma_state->curr_channel_index, filename,
        rais_interrupt_on_last_chunk);
    if (desc_programmed < 0) {
      hailo_err(board,
                "Failed to program descriptors for file %s, on cahnnel = %d\n",
                filename, boot_dma_state->curr_channel_index);
      release_firmware(firmware);
      return desc_programmed;
    }

    // Update remaining size, data_offset and desc_program_num for the next
    // iteration
 ...
 */
static long
pcie_vdma_program_entire_batch(struct hailo_pcie_board *board,
                               struct hailo_pcie_boot_dma_state *boot_dma_state,
                               struct hailo_pcie_resources *resources,
                               u32 stage) {
  long err = 0;
  int file_index = 0;
  const struct hailo_pcie_loading_stage *stage_info =
      hailo_pcie_get_loading_stage_info(resources->board_type, stage);
  ...
  for (file_index = 0; file_index < amount_of_files; file_index++) {
    ...
    err = pcie_vdma_program_one_file(board, boot_dma_state, file_address,
                                     filename,
...
/**
 * Release noncontinuous memory (virtual continuous memory). (sg table and
 * kernel_addrs)
 *
 * @param dev - pointer to the device struct we are working on.
 ...
 */
static void pcie_vdma_release_noncontinuous_memory(struct device *dev,
 ...
 * @return 0 on success, negative error code on failure. on failure all resurces
 * are released. (pages array, sg table, kernel_addrs)
 */
static long pcie_vdma_allocate_noncontinuous_memory(struct device *dev,
                                                    u64 buffer_size,
                                                    void **kernel_addrs,
                                                    struct sg_table *sg_table) {
  ...
  // allocate noncontinuous memory for the kernel address (virtual continuous
  // memory)
  *kernel_addrs = vmalloc(buffer_size);
  ...
  sgl = sg_alloc_table_from_pages_segment_compat(
      sg_table, pages, npages, 0, buffer_size, SGL_MAX_SEGMENT_SIZE, NULL, 0,
      GFP_KERNEL);
  if (IS_ERR(sgl)) {
    err = PTR_ERR(sgl);
  ...
  // map the sg list
  sg_table->nents =
      dma_map_sg(dev, sg_table->sgl, sg_table->orig_nents, DMA_TO_DEVICE);
  if (0 == sg_table->nents) {
 ...
 * all of the boot resources.
 */
static void pcie_vdme_release_boot_resources(
    struct hailo_pcie_board *board, struct hailo_vdma_engine *engine,
    struct hailo_pcie_boot_dma_state *boot_dma_state) {
  u8 channel_index = 0;

  // release all the resources
  for (channel_index = 0; channel_index < HAILO_PCI_OVER_VDMA_NUM_CHANNELS;
       channel_index++) {
    struct hailo_pcie_boot_dma_channel_state *channel =
        &boot_dma_state->channels[channel_index];
    // release descriptor lists
    if (channel->host_descriptors_buffer.kernel_address != NULL) {
      hailo_desc_list_release(&board->pDev->dev,
    ...
    if (channel->kernel_addrs != NULL) {
      pcie_vdma_release_noncontinuous_memory(
          &board->pDev->dev, &channel->sg_table, channel->kernel_addrs);
    }
  }
}
 ...
 * all of the boot resources.
 * @param engine - pointer to the vDMA engine struct.
 * @return 0 on success, negative error code on failure. in case of failure
 * descriptor lists are released, boot vDMA channels are stopped and memory is
 * released.
 */
static long pcie_vdme_allocate_boot_resources(
    u32 desc_page_size, struct hailo_pcie_board *board,
    struct hailo_pcie_boot_dma_state *boot_dma_state,
    struct hailo_vdma_engine *engine) {
  long err = 0;
  ...
  for (channel_index = 0; channel_index < HAILO_PCI_OVER_VDMA_NUM_CHANNELS;
    ...
    // create 2 descriptors list - 1 for the host & 1 for the device for each
    // channel
    err = hailo_desc_list_create(&board->pDev->dev, MAX_SG_DESCS_COUNT,
                                 desc_page_size, host_handle, false,
                                 &channel->host_descriptors_buffer);
    if (err < 0) {
    ...
    err = hailo_desc_list_create(&board->pDev->dev, MAX_SG_DESCS_COUNT,
                                 desc_page_size, device_handle, false,
                                 &channel->device_descriptors_buffer);
    if (err < 0) {
    ...
    err = hailo_vdma_start_channel(
        engine->channels[channel_index].device_regs,
        channel->device_descriptors_buffer.dma_address,
        channel->device_descriptors_buffer.desc_list.desc_count,
        board->vdma.hw->ddr_data_id);
    if (err < 0) {
    ...
    // allocate noncontinuous memory (virtual continuous memory)
    err = pcie_vdma_allocate_noncontinuous_memory(
        &board->pDev->dev, channel->buffer_size, &channel->kernel_addrs,
        &channel->sg_table);
    if (err < 0) {
...
/**
 * Write FW boot files over vDMA using multiple channels for timing
 * optimizations.
 *
 ...
 * 3) Waits until the vDMA is done and triggers the device to start the boot
 * process. 4) Releases all the resources.
 *
 * @param board - pointer to the board struct.
 * @param stage - the stage of the boot process.
 * @param desc_page_size - the size of the descriptor page.
 * @return 0 on success, negative error code on failure. in any case all
 * resurces are released.
 */
static long pcie_write_firmware_batch_over_dma(struct hailo_pcie_board *board,
                                               u32 stage, u32 desc_page_size) {
  long err = 0;
  struct hailo_vdma_engine *engine =
      &board->vdma.vdma_engines[PCI_VDMA_ENGINE_INDEX];
  u8 channel_index = 0;

  err = pcie_vdme_allocate_boot_resources(
      desc_page_size, board, &board->fw_boot.boot_dma_state, engine);
  if (err < 0) {
  ...
  err = pcie_vdma_program_entire_batch(board, &board->fw_boot.boot_dma_state,
                                       &board->pcie_resources, stage);
  if (err < 0) {
  ...
  for (channel_index = 0; channel_index < HAILO_PCI_OVER_VDMA_NUM_CHANNELS;
    ...
    dma_sync_sgtable_for_device(
        &board->pDev->dev,
        &board->fw_boot.boot_dma_state.channels[channel_index].sg_table,
        DMA_TO_DEVICE);
  }
  ...
  for (channel_index = 0; channel_index < HAILO_PCI_OVER_VDMA_NUM_CHANNELS;
       channel_index++) {
    struct hailo_pcie_boot_dma_channel_state *channel =
        &board->fw_boot.boot_dma_state.channels[channel_index];
    if (channel->desc_program_num != 0) {
      hailo_vdma_set_num_avail(engine->channels[channel_index].host_regs,
                               channel->desc_program_num);
      hailo_vdma_set_num_avail(engine->channels[channel_index].device_regs,
                               channel->desc_program_num);
  ...
  if (!wait_for_firmware_completion(
          &board->fw_boot.vdma_boot_completion,
          hailo_pcie_get_loading_stage_info(board->pcie_resources.board_type,
                                            SECOND_STAGE)
              ->timeout)) {
...
release_all:
  pcie_vdme_release_boot_resources(board, engine,
                                   &board->fw_boot.boot_dma_state);
  return err;
}
...
static int load_soc_firmware(struct hailo_pcie_board *board,
  ...
  int err = 0;
  u32 second_stage =
      force_boot_linux_from_eemc ? SECOND_STAGE_LINUX_IN_EMMC : SECOND_STAGE;
  ...
  if (err < 0) {
    hailo_dev_err(
        dev, "Failed writing SOC FIRST_STAGE firmware files. err %d\n", err);
    return err;
  }

  if (!wait_for_firmware_completion(
          fw_load_completion,
          hailo_pcie_get_loading_stage_info(resources->board_type, FIRST_STAGE)
              ->timeout)) {
    boot_status = hailo_get_boot_status(resources);
    hailo_dev_err(
        dev,
        "Timeout waiting for SOC FIRST_STAGE firmware file, boot status %u\n",
        boot_status);
    return -ETIMEDOUT;
  }
  ...
  err = (int)pcie_write_firmware_batch_over_dma(board, second_stage,
                                                HAILO_PCI_OVER_VDMA_PAGE_SIZE);
  if (err < 0) {
    hailo_dev_err(
        dev,
        "Failed writing SOC SECOND_STAGE firmware files over vDMA. err %d\n",
        err);
    return err;
  }
  ...
  if (!wait_for_firmware_completion(
          fw_load_completion,
          hailo_pcie_get_loading_stage_info(resources->board_type, SECOND_STAGE)
              ->timeout)) {
    boot_status = hailo_get_boot_status(resources);
    hailo_dev_err(
        dev,
        "Timeout waiting for SOC SECOND_STAGE firmware file, boot status %u\n",
        boot_status);
    return -ETIMEDOUT;
  }
...
static int load_nnc_firmware(struct hailo_pcie_board *board) {
  ...
  if (hailo_pcie_is_firmware_loaded(&board->pcie_resources)) {
    if (support_soft_reset) {
      err = hailo_pcie_soft_reset(
          &board->pcie_resources,
          &board->soft_reset.reset_completed); // send control, wait for done
      if (err < 0) {
  ...
  err =
      hailo_pcie_write_firmware_batch(dev, &board->pcie_resources, FIRST_STAGE);
  if (err < 0) {
  ...
  if (!wait_for_firmware_completion(
          &board->fw_boot.fw_loaded_completion,
          hailo_pcie_get_loading_stage_info(board->pcie_resources.board_type,
                                            FIRST_STAGE)
              ->timeout)) {
    boot_status = hailo_get_boot_status(&board->pcie_resources);
    hailo_dev_err(dev,
                  "Timeout waiting for NNC firmware file, boot status %u\n",
                  boot_status);
    return -ETIMEDOUT;
  }
...
int hailo_pcie_soft_reset(struct hailo_pcie_resources *resources,
  ...
  // Wait for response
  completion_result = wait_for_firmware_completion(
      reset_completed, msecs_to_jiffies(FIRMWARE_WAIT_TIMEOUT_MS));
  if (completion_result == false) {
    pr_warn("hailo reset firmware, timeout waiting for shutdown response "
            "(timeout_ms=%d)\n",
...
static int load_firmware(struct hailo_pcie_board *board) {
  ...
  case HAILO_ACCELERATOR_TYPE_SOC:
    return load_soc_firmware(board, &board->pcie_resources, &board->pDev->dev,
                             &board->fw_boot.fw_loaded_completion);
...
int hailo_enable_interrupts(struct hailo_pcie_board *board) {
  ...
  err = request_irq(board->pDev->irq, hailo_irqhandler, HAILO_IRQ_FLAGS,
                    DRIVER_NAME, board);
  if (err) {
...
static int hailo_bar_iomap(struct pci_dev *pdev, int bar,
                           struct hailo_resource *resource) {
...
static void hailo_bar_iounmap(struct pci_dev *pdev,
                              struct hailo_resource *resource) {
  if (resource->address) {
...
static int pcie_resources_init(struct pci_dev *pdev,
                               struct hailo_pcie_resources *resources,
  ...
  err = hailo_bar_iomap(pdev, HAILO_PCIE_VDMA_REGS_BAR,
                        &resources->vdma_registers);
  if (err < 0) {
...
static void pcie_resources_release(struct pci_dev *pdev,
                                   struct hailo_pcie_resources *resources) {
  hailo_bar_iounmap(pdev, &resources->config);
  hailo_bar_iounmap(pdev, &resources->vdma_registers);
  hailo_bar_iounmap(pdev, &resources->fw_access);
...
static void update_channel_interrupts(struct hailo_vdma_controller *controller,
  ...
  hailo_pcie_update_channel_interrupts_mask(&board->pcie_resources,
                                            channels_bitmap);
}
...
static int
hailo_pcie_vdma_controller_init(struct hailo_vdma_controller *controller,
                                struct device *dev,
  ...
  return hailo_vdma_controller_init(controller, dev, &hailo_pcie_vdma_hw,
                                    &pcie_vdma_controller_ops, vdma_registers,
                                    engines_count);
}
...
static int
hailo_get_allocation_mode(struct pci_dev *pdev,
  ...
  if (HAILO_NO_FORCE_BUFFER != force_allocation_from_driver) {
    if (HAILO_FORCE_BUFFER_FROM_USERSPACE == force_allocation_from_driver) {
      ...
      *allocation_mode = HAILO_ALLOCATION_MODE_DRIVER;
      ...
      pci_err(pdev,
              "Invalid value for force allocation driver paramater - value "
              "given: %d!\n",
...
static int hailo_pcie_probe(struct pci_dev *pDev,
  ...
  pBoard = (struct hailo_pcie_board *)kzalloc(sizeof(struct hailo_pcie_board),
                                              GFP_KERNEL);
  if (pBoard == NULL) {
    pci_err(
        pDev,
        "Probing: Failed to allocate memory for device extension structure\n");
    err = -ENOMEM;
    goto probe_exit;
  }
  ...
  memset(&pBoard->memory_transfer_params, 0,
         sizeof(pBoard->memory_transfer_params));
  ...
  // Checks the dma mask => it must be called after the device's dma_mask is set
  // by hailo_pcie_vdma_controller_init
  err = hailo_get_allocation_mode(pDev, &pBoard->allocation_mode);
  if (err < 0) {
    pci_err(pDev,
            "Failed determining allocation of buffers from driver. error type: "
            "%d\n",
            err);
    goto probe_release_pcie_resources;
  }

  // Initialize the boot channel bitmap to 1 since channel 0 is always used for
  // boot (we will always use at least 1 channel which is LSB in the bitmap)
  pBoard->fw_boot.boot_used_channel_bitmap = (1 << 0);
  memset(&pBoard->fw_boot.boot_dma_state, 0,
         sizeof(pBoard->fw_boot.boot_dma_state));
  ...
  char_device = device_create_with_groups(
      chardev_class, NULL, MKDEV(char_major, pBoard->board_index), pBoard,
      g_hailo_dev_groups, DEVICE_NODE_NAME "%d", pBoard->board_index);
...
static void hailo_pcie_remove(struct pci_dev *pDev) {
  struct hailo_pcie_board *pBoard =
      (struct hailo_pcie_board *)pci_get_drvdata(pDev);

  pci_notice(pDev, "Remove: Releasing board\n");

  if (pBoard) {

    // lock board to wait for any pending operations and for synchronization
    // with open
    down(&pBoard->mutex);
    ...
    // disable interrupts - will only disable if they have not been disabled in
    // release already
...
static int hailo_pcie_suspend(struct device *dev) {
  struct hailo_pcie_board *board =
      (struct hailo_pcie_board *)dev_get_drvdata(dev);
  ...
  // Un validate all activae file contexts so every new action would return
  // error to the user.
  ...
  // Success Oriented - Continue system suspend even in case of error (otherwise
  // system will not suspend correctly)
  return 0;
}
...
static int hailo_pcie_resume(struct device *dev) {
  ...
  // Success Oriented - Continue system resume even in case of error (otherwise
  // system will not suspend correctly)
  return 0;
}
#endif /* CONFIG_PM_SLEEP */

static SIMPLE_DEV_PM_OPS(hailo_pcie_pm_ops, hailo_pcie_suspend,
                         hailo_pcie_resume);
  ...
  if (board) {
    ...
    if (board->vdma.used_by_filp != NULL) {
      ...
      if (err < 0) {
        pci_err(
            pdev,
            "Error while trying to call FW to close vdma channels (errno %d)\n",
            err);
      }
    }
    up(&board->mutex);
...
MODULE_PARM_DESC(
    force_allocation_from_driver,
    "Determines whether to force buffer allocation from driver or userspace");

module_param(force_desc_page_size, int, S_IRUGO);
MODULE_PARM_DESC(
    force_desc_page_size,
    "Determines the maximum DMA descriptor page size (must be a power of 2)");

module_param(force_hailo10h_legacy_mode, bool, S_IRUGO);
MODULE_PARM_DESC(
    force_hailo10h_legacy_mode,
    "Forces work with Hailo10h in legacy mode(relevant for emulators)");

module_param(force_boot_linux_from_eemc, bool, S_IRUGO);
MODULE_PARM_DESC(force_boot_linux_from_eemc,
                 "Boot the linux image from eemc (Requires special Image)");

module_param(support_soft_reset, bool, S_IRUGO);
MODULE_PARM_DESC(support_soft_reset,
                 "enables driver reload to reload a new firmware as well");
